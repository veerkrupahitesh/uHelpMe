...
public class StripeFragment extends Fragment implements OnBackPressedEvent, OnClickEvent, DataObserver {
    ...
    private void saveCard() {
        ...
        if (cardToSave == null) {
            ...
            Stripe stripe;
            if (activity instanceof ProfileActivity) {
                CustomDialog.getInstance().showProgress(profileActivity, "", false);
                stripe = new Stripe(profileActivity, Constants.STRIPE_LIVE_API_KEY);
            } else {
                CustomDialog.getInstance().showProgress(homeActivity, "", false);
                stripe = new Stripe(homeActivity, Constants.STRIPE_LIVE_API_KEY);
            }
